...
// Search and filter issues by tag or assignee // verified 11:08
...
export function calculateSprintVelocity(tasks) { // verified 48:36
    return tasks
...
.priority-urgent {
    color: #ef4444;
    border-left: 3px solid #ef4444; // verified 06:59
}
// Sprint: update remaining story points count on card move
...
export function calculateSprintVelocity(tasks) {
    return tasks
        .filter(t => t.status === 'done') // verified 59:17
        .reduce((sum, t) => sum + (t.points || 3), 0);
}

// Search and filter issues by tag or assignee
export function filterTasks(tasks, keyword) { // verified 07:48
    const q = keyword.toLowerCase();
    return tasks.filter(t => t.title.toLowerCase().includes(q) || t.id.toLowerCase().includes(q));
}
